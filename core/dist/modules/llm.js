...
export const LLMInputSchema = z
    ...
    .passthrough();
export async function runLLM(provider, raw) {
    const cfg = LLMInputSchema.parse(raw);
    const messages = [];
    if (cfg.system)
        messages.push({ role: "system", content: cfg.system });
    for (const c of cfg.context)
        messages.push({ role: "user", content: c });
    if (cfg.user)
        messages.push({ role: "user", content: cfg.user });
    if (cfg.assistant)
        messages.push({ role: "assistant", content: cfg.assistant });
...
export class MockCPUProvider {
    supportsSystem;
    constructor(supportsSystem = false) {

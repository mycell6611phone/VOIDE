...
# /core/src/proto/voide — Legacy VOIDE Artifacts
...
Legacy namespace-specific generated files. There is no longer an automated
regeneration step; the folder should trend toward deletion. If you must touch
these files, treat them as read-only snapshots and build adapters elsewhere.

## Backend Transition Notes

- When replacing legacy schemas, describe their role (Build vs. Run) here so downstream packages know how to consume them.
- Keep a changelog snippet in this file whenever you shim an old format so teams can coordinate cleanup.

    ...
    for (const edge of flow.edges ?? []) {
        ...
        if (toNode) {
            ...
            if (ins)
                ...
                incoming.set(toNode, [edge]);
        }
    }
    return { incoming, outgoing };
}
function clonePortPayloadMap(map) {
    if (!map)
        return {};
    const clone = {};
...
function recordEmission(store, port, payload, nodeId) {
    if (!port) {
...
function mergeReturnedOutputs(result, store, nodeId) {
    if (result == null)
        return;
    if (Array.isArray(result)) {
        for (const entry of result) {
            ...
            if (Array.isArray(entry) && entry.length >= 2) {
                const [port, payload] = entry;
                recordEmission(store, String(port), payload, nodeId);
                continue;
            }
            if (typeof entry === "object") {
                const port = entry.port ?? (Array.isArray(entry) ? entry[0] : undefined);
                const payload = entry.payload ?? entry.value ?? (Array.isArray(entry) ? entry[1] : undefined);
...
function normalizeEntryData(entryData) {
    ...
    const process = (nodeId, value) => {
        ...
        if (typeof value === "object" && value !== null && !Array.isArray(value)) {
            for (const [port, payload] of Object.entries(value)) {
                assign(String(nodeId), port, payload);
            }
            return;
        }
        console.warn(`[runtime] Ignoring entry payload for node "${nodeId}"; expected a map of ports to payloads.`);
    };
    if (entryData instanceof Map) {
        ...
        for (const [nodeId, payloads] of Object.entries(entryData)) {
...
export async function runFlow(runtimeFlow, entryData) {

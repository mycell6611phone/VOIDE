...
function ensureArray<T>(map: Map<string, T[]>, key: string): T[] {
...
export function topoOrder(flow: RuntimeFlow): string[] {
  ...
  if (order.length !== nodeIds.length) {
    console.warn(
      `[runtime] topoOrder detected a cycle or missing nodes. Expected ${nodeIds.length}, got ${order.length}.`,
    );
  }

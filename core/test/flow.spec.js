...
const execOptions = { cwd: coreRoot };
describe("flow schema", () => {
    ...
    it("compile deterministic", () => {
        ...
        expect(Buffer.from(bin1).toString("hex")).toBe(Buffer.from(bin2).toString("hex"));
    ...
    it("cli validate success and failure", async () => {
        ...
        await expect(exec(nodeBin, [...nodeLoaderArgs, "validate", bad], execOptions)).rejects.toHaveProperty("code", 1);
    }, CLI_TIMEOUT);
    it("cli pack produces output", async () => {
        ...
        await exec(nodeBin, [...nodeLoaderArgs, "pack", src, "--out", out], execOptions);

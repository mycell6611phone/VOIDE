...
import { runFlow, TelemetryEventType, initTelemetry, shutdownTelemetry } from "../src/run";
...
function readFrames(ringPath: string): { frames: Frame[]; heartbeat: bigint } {
...
describe("telemetry ring", () => {
  it("captures scheduler events in shared ring", async () => {
    ...
    const starts = frames
      .filter((f) => f.type === TelemetryEventType.NodeStart)
      .map((f) => f.payload.id);
    expect(starts).toEqual(["user", "prompt", "llm", "out"]);

    const ends = frames
      .filter((f) => f.type === TelemetryEventType.NodeEnd)
    ...
    const wires = frames
      .filter((f) => f.type === TelemetryEventType.WireTx)

...
export function setupIPC() {
    ...
    ipcMain.handle("voide:saveFlow", async (_e, { flow, filePath }) => {
        if (!validate(flow))
            return { error: ajv.errorsText(validate.errors) };
        const db = getDB();
    ...
    ipcMain.handle("voide:validateFlow", async (_e, flow) => {
    ...
    ipcMain.handle("voide:runFlow", async (_e, { flow }) => runFlow(flow));

...
export type CanvasPlan = {
  ...
  order: string[];
  entry: string;
  ops: Record<string, { type: string; params: Record<string, unknown> }>;
  routes: Array<{
    id: string;
...
export type CanvasRunPayload = {
  plan: CanvasPlan | (CanvasPlan & { hash: string });
...
export type CanvasRunResult = {
  ...
  hash?: string;
  runId?: string;
  outputs?: Awaited<ReturnType<typeof getLastRunPayloads>>;
  error?: string;
};
...
function normalizePortTypes(port: CanvasPort | undefined): string[] {
  if (!port) return [];
  const raw: unknown = port.dtype ?? port.types;
...
function portIndexFor(nodes: CanvasNode[]): Map<string, PortIndex> {
  const index = new Map<string, PortIndex>();
  for (const node of nodes) {
    const record: PortIndex = {
    ...
    for (const dir of ["in", "out"] as const) {
      ...
      for (const entry of ports) {
        ...
        const id = normalizePortId(entry as CanvasPort);
        if (!id) continue;
        record[dir].set(id, normalizePortTypes(entry as CanvasPort));
      }
    }
...
function normalizeEdgeEndpoint(endpoint: CanvasEdgeEndpoint, fallbackNode?: string): {
  node: string;
  port: string;
} | null {
  const nodeId =
    (typeof endpoint.node === "string" && endpoint.node.trim().length > 0 && endpoint.node.trim()) ||
    ...
    fallbackNode;
  const portId =
    (typeof endpoint.port === "string" && endpoint.port.trim().length > 0 && endpoint.port.trim()) ||
...
function computeGraphHash(canvas: CanvasGraph): string {
  const simplified = {
    id: canvas.id ?? null,
...
export function isCanvasBuildInput(payload: unknown): payload is CanvasGraph {
...
export function isCanvasRunPayload(payload: unknown): payload is CanvasRunPayload {
  if (!payload || typeof payload !== "object") return false;
  const candidate = payload as { plan?: unknown };
  if (!candidate.plan || typeof candidate.plan !== "object") return false;
  const plan = candidate.plan as { hash?: unknown; order?: unknown };
  if (typeof plan.hash !== "string" || plan.hash.trim().length === 0) return false;
  if (!Array.isArray(plan.order)) return false;
  return true;
}

function validateCanvas(canvas: CanvasGraph): { errors: string[]; order: string[]; entry: string | null } {
  const errors: string[] = [];
  const nodeIndex = new Map<string, CanvasNode>();
  ...
  for (const edge of canvas.edges ?? []) {
    ...
    const sourcePorts = portIndex.get(from.node)?.out ?? new Map<string, string[]>();
    if (!sourcePorts.has(from.port)) {
      errors.push(`edge:${edgeId} source port ${from.node}.${from.port} missing`);
    }
    const targetPorts = portIndex.get(to.node)?.in ?? new Map<string, string[]>();
    ...
    if (sourcePorts.has(from.port) && targetPorts.has(to.port)) {
      ...
      if (
        ...
        errors.push(
          `edge:${edgeId} incompatible port types ${from.node}.${from.port} -> ${to.node}.${to.port}`,
        );
      }
    }
...
function toFlowGraph(canvas: CanvasGraph): FlowGraph {
  const nodes = (canvas.nodes ?? []).map((node) => ({
    ...
    in: toArray(node.in)
      .map((port) => ({
        port: normalizePortId(port as CanvasPort) ?? "",
        types: normalizePortTypes(port as CanvasPort),
      }))
  ...
  const edges = canvas.edges?.map((edge, index) => {
    ...
    return {
      ...
      from: [from.node, from.port] as [string, string],
      to: [to.node, to.port] as [string, string],
      label: undefined,
    };
  }) ?? [];
...
function createPlan(
  canvas: CanvasGraph,
  metadata: { order: string[]; entry: string | null },
  hash: string,
): CanvasPlan {
  const ops: CanvasPlan["ops"] = {};
  for (const node of canvas.nodes ?? []) {
    ops[node.id] = {
      type: node.type,
      params: { ...(node.params ?? {}) },
...
async function ensureFlowDirectory(hash: string): Promise<string> {
...
function runnableModuleSource(plan: CanvasPlan): string {
...
async function persistArtifacts(dir: string, plan: CanvasPlan, compiled: ReturnType<typeof compileAndCache>["entry"]): Promise<void> {
...
async function loadFlowFromDisk(dir: string): Promise<FlowDef | null> {
  const flowPath = path.join(dir, "flow.json");
  try {
...
export async function buildCanvasFlow(canvas: CanvasGraph): Promise<CanvasBuildResult> {
...
export async function runCanvasFlow(payload: CanvasRunPayload): Promise<CanvasRunResult> {
  try {
    ...
    let flow: FlowDef | null = compiled?.flow ?? null;
    if (!flow) {

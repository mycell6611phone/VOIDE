...
export async function runFlow(flow: FlowDef) {
  ...
  const st: RunState = { runId, flow, frontier: f0, halted: false, iter: new Map(), values: new Map() };
  runs.set(runId, st);
...
async function executeNode(st: RunState, node: NodeDef): Promise<Array<[string, any]>> {
  ...
  if (node.type === "llm.generic" || node.type === "critic") {
    ...
    if (params.adapter !== "mock") {
      ...
      if (!model) {
        const dt = Date.now() - t0;
        await recordRunLog({ runId: st.runId, nodeId: node.id, tokens: 0, latencyMs: dt, status: 'error', error: 'model not found' });

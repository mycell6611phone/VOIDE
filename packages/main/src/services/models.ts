...
export interface RegistryResponse {
  models: (RegistryModel & { status: ModelStatus })[];
}
...
function getLocalSource(m: RegistryModel, regDir: string): string | null {
  ...
  if (path.isAbsolute(m.url)) return m.url;
...
export async function installModel(id: string, onProgress?: (p: { id: string; loaded: number; total: number }) => void) {
  const reg = await getModelRegistry();
  const m = reg.models.find(r => r.id === id);
  if (!m) throw new Error(`model ${id} not found`);
  if (m.status === "blocked-license") throw new Error("blocked-license");
  const regDir = path.dirname(registryPath());
  const src = getLocalSource(m, regDir);
  if (!src) throw new Error("unavailable-offline");

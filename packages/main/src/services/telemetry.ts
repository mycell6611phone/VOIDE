...
class RingTransport implements TelemetryTransport {
  public kind: "ring" = "ring";
  private fd: number;
  private capacity: number;
  private writeIdx = 0;
  private readIdx = 0;
  private dropped = 0;
  private heartbeatBuffer = Buffer.allocUnsafe(8);
  private idxBuffer = Buffer.allocUnsafe(8);
  private dropBuffer = Buffer.allocUnsafe(4);

  constructor(private ringPath: string, sizeMB: number) {
    if (sizeMB <= 0) {
  ...
  private writeRing(buffer: Buffer, offset: number) {
    let cursor = offset;
  ...
  private peekFrameLength(offset: number): number {
...
class SocketTransport implements TelemetryTransport {
  public kind: "uds" | "udp";
  ...
  emit(frame: Uint8Array): boolean {
    ...
    try {
      if (this.kind === "udp") {
        ...
        (this.socket as any).send(buffer, 0, buffer.length, this.udsPath!);
      }
      return true;
    } catch (error) {
...
function resolveDefaultRingPath(): string {
...
export function emitSchedulerTelemetry(event: TelemetryEventInput): boolean {
  if (!transport) {
    return false;
  }
  try {
...
export function telemetryTransportKind(): "ring" | "uds" | "udp" | "none" {

...
export interface ModelRecord {
  id: string;
  name: string;
  backend: "llamacpp" | "gpt4all" | "ollama";
  sources: string[];             // download mirrors
  filename: string;
  sha256: string;                // expected hash
  sizeBytes: number;
  params?: { context?: number; quant?: string; maxTokens?: number; minVRAMGB?: number; minRAMGB?: number };
}

export interface SelectOptions {
  modelsDir?: string;            // default ~/.voide/models
  verifyOnUse?: "never" | "if-registry-changed" | "always";
  registryHash?: string;         // hash of models.json for change detection
}

export class ModelManager {
  constructor(private getRegistry: () => Promise<ModelRecord[]>) {}

  async select(id: string, opts: SelectOptions = {}): Promise<string> {
    const home = process.env.HOME || process.env.USERPROFILE || ".";
    const baseDir = opts.modelsDir ?? path.join(home, ".voide", "models");
    const registry = await this.getRegistry();
    const rec = registry.find(r => r.id === id);
    if (!rec) throw new Error(`Model ${id} not found in registry`);
    ...
    switch (opts.verifyOnUse ?? "if-registry-changed") {
      case "always": mustVerify = true; break;
      case "if-registry-changed":
        try {
          const current = await fs.readFile(shaPath, "utf8");
          if (!current.includes(rec.sha256)) mustVerify = true;
        } catch { mustVerify = true; }
        break;
      case "never": mustVerify = false; break;
    }
  ...
  private async downloadWithMirrors(rec: ModelRecord, dst: string, shaPath: string) {
    ...
    for (const url of rec.sources) {
      try {
        ...
        if (hash !== rec.sha256) throw new Error(`checksum mismatch`);
  ...
  private async preflightCompute(rec: ModelRecord) {

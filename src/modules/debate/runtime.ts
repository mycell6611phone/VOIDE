import { DebateConfig, DebateFormat } from "./debateConfig";
...
export function setLlmRequest(fn: typeof _llmRequest) {
  _llmRequest = fn;
}

export const llmRequest: typeof _llmRequest = (prompt, opts) =>
  _llmRequest(prompt, opts);

function defaultPrompt(format: DebateFormat, input: string): string {
  switch (format) {

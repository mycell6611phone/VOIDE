...
import { DEFAULT_PROMPT_PRESET_ID, PROMPT_PRESETS, PROMPT_PRESET_MAP, inferPromptPresetFromText, } from "@voide/shared";
import { promptConfigFromBytes, promptConfigToBytes, } from "./promptConfig";
const nodeStyles = {
    width: 140,
    padding: 8,
    borderRadius: 6,
...
function normalizeForSave(config) {
    let presetId = config.preset in PROMPT_PRESET_MAP
        ? config.preset
        : DEFAULT_PROMPT_PRESET_ID;
    const to = config.to === "system" ? "system" : "user";
...
export function PromptModuleNode({ id, config, selected = false, onConfigure, onMenuOpen, onMenuClose, }) {
    const [storedConfig, setStoredConfig] = useState(() => normalizeForSave(promptConfigFromBytes(config ?? null)));
    const [menuOpen, setMenuOpen] = useState(false);
    const [draft, setDraft] = useState(() => cloneConfig(storedConfig));
    ...
    const summary = useMemo(() => {
        ...
        if (!trimmed.length && storedConfig.preset !== "custom") {
            const presetText = (PROMPT_PRESET_MAP[storedConfig.preset]?.defaultText ?? "").trim();
    ...
    function selectPreset(nextId) {
        setDraft((prev) => {
            const resolvedId = nextId in PROMPT_PRESET_MAP ? nextId : DEFAULT_PROMPT_PRESET_ID;
            const preset = PROMPT_PRESET_MAP[resolvedId];
            return {
                ...prev,
                preset: resolvedId,
                text: resolvedId === "custom"
                    ? prev.text
    ...
    function handleTextChange(value) {
        setDraft((prev) => {
            let candidatePreset = prev.preset in PROMPT_PRESET_MAP
                ? prev.preset
                : DEFAULT_PROMPT_PRESET_ID;
            const trimmed = value.trim();
            const presetText = candidatePreset !== "custom"
    ...
    return (<div style={{
      ...
      {menuOpen && (<div style={{
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={presetListStyle}>
              {PROMPT_PRESETS.map((preset) => {
                ...
                return (<button key={preset.id} type="button" onClick={() => selectPreset(preset.id)} aria-pressed={isActive} style={{
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={radioGroupStyle}>
              <label style={radioLabelStyle}>
                <input type="radio" name={`prompt-placement-${id}`} value="user" checked={draft.to === "user"} onChange={() => handlePlacementChange("user")} style={radioInputStyle}/>
                User message
              </label>
              <label style={radioLabelStyle}>
                <input type="radio" name={`prompt-placement-${id}`} value="system" checked={draft.to === "system"} onChange={() => handlePlacementChange("system")} style={radioInputStyle}/>
                System prompt
              </label>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea ref={textareaRef} value={draft.text} onChange={(event) => handleTextChange(event.target.value)} placeholder="Custom prompt here.." style={textareaStyle}/>
          ...
          <div style={actionRowStyle}>
            ...
            <button type="button" onClick={closeMenu} style={{
                padding: "6px 10px",
                borderRadius: 4,

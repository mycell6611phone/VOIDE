...
import { defaultPromptConfig, promptConfigFromBytes, promptConfigToBytes, } from "../src/modules/prompt/promptConfig";
...
describe("promptConfig conversion", () => {
    ...
    it("coerces unknown presets to custom", () => {
        const cfg = promptConfigFromBytes(toBytes({ text: "Hello", preset: "assistant", to: "user" }));
        expect(cfg.preset).toBe("custom");
        expect(cfg.text).toBe("Hello");
    ...
    it("encodes state with passthrough merged", () => {
        const start = promptConfigFromBytes(toBytes({ text: "A", preset: "custom", to: "user", extra: "value" }));
        start.text = "Updated";
        start.to = "system";
        start.preset = "custom";
    ...
    it("round-trips recognized presets", () => {
        const initial = promptConfigFromBytes(toBytes({ text: "", to: "user", preset: "analysis" }));
